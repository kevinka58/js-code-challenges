...
function sayHello() {
  ...
  function computeRemainder(num, num2) {
  ...
  function range(int, int2){
    if (start > finish) return 'Fisrt argument must be less than the second'
      arr = []
    for (var i = int; i < int2; i++){
      arr.push(i);
    }
    return arr;
  }
  ...
  function reverseUpcaseString(string) {
    return splitString = string.split("").reverse().join('').toUpperCase()
  }
  ...
  function formatWithPadding(int, char, int){
  ...
  // Your solution for 12-hammingDistance here:
    ...
    function hammingDistance(string, string2){
      if (string !== string2.length) return NaN
      let count = 0;
      for (let i = 0; i < string.length; i++) {
        if(string.charAt(i) !== string2.charAt(i)) count++;
      }
      return count
    }
  ...
  function reduceArray(arr, callback, accumulator) {
    var accumulator = initAcc;
    arr.forEach(function(el, idx) {
      acc = callback(accumulator, el, idx);
    });
    return accumulator;
  }
